...
// --- KOMPONEN INPUT DIPINDAHKAN KELUAR (FIXED) ---
// Komponen ini harus menerima value dan onChange dari props agar tidak re-render total
const FormInput = ({ 
  label, 
  value, 
  onChange, 
...
const initialData: MasterDataItem[] = [
  { id: 1, kode: "DEP001", nama: "Departemen IT", deskripsi: "Bagian Teknologi Informasi", status: "Aktif", createdAt: "2024-01-15" },
  { id: 2, kode: "DEP002", nama: "Departemen HR", deskripsi: "Bagian Sumber Daya Manusia", status: "Aktif", createdAt: "2024-01-16" },
  { id: 3, kode: "DEP003", nama: "Departemen Keuangan", deskripsi: "Bagian Keuangan", status: "Non-Aktif", createdAt: "2024-01-17" },
]

const defaultFormData: MasterDataItem = { kode: "", nama: "", deskripsi: "", status: "Aktif" }
...
export default function Ketentuan() {
  ...
  const [formData, setFormData] = useState<MasterDataItem>(defaultFormData)
  const [alert, setAlert] = useState<{ open: boolean; message: string; variant: "success" | "error" | "info" }>({ open: false, message: "", variant: "success" })
  ...
  const handleFilterChange = (value: string) => { setFilter(value); setCurrentPage(1) }
  ...
  const handleSubmit = () => {
    if (!formData.kode.trim() || !formData.nama.trim()) {
      showAlert("Kode dan Nama wajib diisi!", "error"); return
    }

    const isAdd = popup.mode === "add"

    if (isAdd) {
      setData([...data, { id: Date.now(), ...formData, createdAt: new Date().toISOString().split("T")[0] }])
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Ketentuan</h1>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white rounded-3xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-shadow">
              <div className="flex items-center justify-between">
                ...
                <div className="p-4 bg-blue-100 rounded-2xl"><Database className="w-8 h-8 text-blue-600" /></div>
              </div>
            </div>
            <div className="bg-white rounded-3xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-shadow">
              <div className="flex items-center justify-between">
                ...
                <div className="p-4 bg-green-100 rounded-2xl"><CheckCircle className="w-8 h-8 text-green-600" /></div>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gradient-to-r from-gray-50 to-gray-100 border-b-2 border-gray-200">
                  <tr>
                    {["Kode", "Nama", "Deskripsi", "Status", "Dibuat", "Aksi"].map(h => (
                  ...
                  <AnimatePresence>
                    {paginatedData.length > 0 ? (
                      paginatedData.map((item, index) => (
                        ...
                        >
                          <td className="p-4"><span className="font-mono font-semibold text-gray-800 bg-gray-100 px-3 py-1 rounded-lg">{item.kode}</span></td>
                      ...
                      <tr>
                        <td colSpan={6} className="p-12 text-center">
                          <div className="flex flex-col items-center gap-3">
                            <div className="p-4 bg-gray-100 rounded-full"><Database className="w-12 h-12 text-gray-400" /></div>
          ...
          <AnimatePresence>
            {popup.open && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={closePopup}>
                <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: "spring", duration: 0.5 }} className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
                  {/* VIEW MODE */}
                  {popup.mode === "view" && popup.data && (
                    <>
                      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-6 text-white flex items-center justify-between">
                        <div className="flex items-center gap-3"><div className="p-2 bg-white/20 rounded-xl backdrop-blur-sm"><Eye className="w-6 h-6" /></div><h2 className="text-2xl font-bold">Detail Data</h2></div>
                        <button onClick={closePopup} className="p-2 hover:bg-white/20 rounded-xl transition-colors"><X className="w-6 h-6" /></button>
                      </div>
                      <div className="p-6 space-y-4">
                        {[{ label: "Kode", value: popup.data.kode }, { label: "Nama", value: popup.data.nama }].map((item) => (
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-gray-50 rounded-2xl p-4"><p className="text-xs text-gray-500 mb-1">Status</p><p className="font-semibold text-gray-800">{popup.data.status}</p></div>
                          <div className="bg-gray-50 rounded-2xl p-4"><p className="text-xs text-gray-500 mb-1">Dibuat</p><p className="font-semibold text-gray-800">{popup.data.createdAt}</p></div>
                        </div>
                  ...
                  {(popup.mode === "add" || popup.mode === "edit") && (
                    <>
                      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-6 text-white flex items-center justify-between">
                        <div className="flex items-center gap-3"><div className="p-2 bg-white/20 rounded-xl backdrop-blur-sm">{popup.mode === "add" ? <Plus className="w-6 h-6" /> : <Edit2 className="w-6 h-6" />}</div><h2 className="text-2xl font-bold">{popup.mode === "add" ? "Tambah Data" : "Edit Data"}</h2></div>
                        <button onClick={closePopup} className="p-2 hover:bg-white/20 rounded-xl transition-colors"><X className="w-6 h-6" /></button>
                      </div>
                      <div className="p-6 space-y-4">
                        <FormInput label="Kode" value={formData.kode} onChange={(val) => handleInputChange("kode", val)} placeholder="Masukkan kode" />
                        <FormInput label="Nama" value={formData.nama} onChange={(val) => handleInputChange("nama", val)} placeholder="Masukkan nama" />
                        <FormInput label="Deskripsi" value={formData.deskripsi} onChange={(val) => handleInputChange("deskripsi", val)} placeholder="Masukkan deskripsi" type="textarea" rows={3} />
                        <FormInput label="Status" value={formData.status} onChange={(val) => handleInputChange("status", val)} placeholder="Pilih Status" type="select" />
                  ...
                  {popup.mode === "delete" && popup.data && (
                    <>
                      <div className="bg-gradient-to-r from-red-500 to-pink-600 p-6 text-white flex items-center justify-between">
                        <div className="flex items-center gap-3"><div className="p-2 bg-white/20 rounded-xl backdrop-blur-sm"><Trash2 className="w-6 h-6" /></div><h2 className="text-2xl font-bold">Hapus Data</h2></div>
                        <button onClick={closePopup} className="p-2 hover:bg-white/20 rounded-xl transition-colors"><X className="w-6 h-6" /></button>
                      </div>
                      <div className="p-6">
                        <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-6 mb-6">
                          <p className="text-gray-700 text-center">Apakah Anda yakin ingin menghapus data <span className="font-bold text-red-600">{popup.data.nama}</span>?</p>
          ...
          <AnimatePresence>
            {alert.open && (
              <motion.div initial={{ opacity: 0, x: 200 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 200 }} transition={{ type: "spring", stiffness: 120, damping: 18 }} className="fixed right-6 top-6 z-50">
                <div className={`max-w-sm px-4 py-3 rounded-2xl shadow-lg text-white flex items-center gap-3 ${alert.variant === "success" ? "bg-green-600" : alert.variant === "error" ? "bg-red-600" : "bg-blue-600"}`}>
                  <div className="font-medium">{alert.message}</div>
                  <button onClick={() => setAlert(s => ({ ...s, open: false }))} className="ml-2 bg-white/20 px-2 py-1 rounded-md hover:bg-white/30" aria-label="Close alert">Tutup</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
